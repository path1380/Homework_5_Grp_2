...
system("make -f Makefile_derivative clean");
for($q = 2;$q <= $num_quad_max; $q = $q+1){
	...
	close( OUTFILE );
	close( FILE );
	system("make -f Makefile_derivative");
	system("./derivative2d.x >> output.txt");
	system("make -f Makefile_derivative clean");
}
system("matlab \"$@\" -nosplash -nodisplay < build/derivative_plot.m");
