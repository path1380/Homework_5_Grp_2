program annulus_GH_copy_for_coeffs
...
!Testing set_initial_data
  call allocate_quad(quad_test_init, q_test_init, n_gll_test_init, 2)
  quad_test_init%q = q_test_init
  quad_test_init%nvar = 2
  quad_test_init%n_gll = n_gll_test_init

!Co-ordinates of test element
  quad_test_init%xy(1,:) = (/0.5_dp,1.0_dp/)
  quad_test_init%xy(2,:) = (/0.5_dp, -1.0_dp/)
  quad_test_init%xy(3,:) = (/-0.5_dp,-1.0_dp/)
  quad_test_init%xy(4,:) = (/-0.5_dp,1.0_dp/)
  ...
  do i=0,nint
     do j=0,nint
        ...
        error(i,j) = ABS(exact_val(i,j)-approximation(i,j))
     end do
  end do
  call deallocate_quad(quad_test_init)
  write(*,*) error

contains

  subroutine set_initial_data(qd,nint,uhat_2D)
    ! Call set_metric
    ! Call init_data and evaluate it at the nodes, store as matrix
    ! Get weights, perform G-L quadrature from -1 to 1
    ! Divide by norms of legendre functions
    integer :: nint
    type(quad) :: qd
    ...
    call set_metric(qd, xnodes, diffmat, nint)
!stop 123
    x = qd%x
    y = qd%y
    do i=0,nint
       do j=0,nint
          ...
          !write(*,*) legendre(xnodes(j),i)
          !write(*,*) init_u(xnodes(i),xnodes(j))
       end do
       !write(*,*) uhat_2D(:,i)
       !write(*,*) y(i,:)
    end do
...
end program annulus_GH_copy_for_coeffs
